package com.javeriana.eventos.inscription.infrastructure.retention;
...
public class RetentionService {
    ...
    @SchedulerLock(name = "inscription-RetentionService",
                   lockAtMostFor = "PT30M",
                   lockAtLeastFor = "PT1M")
    public void limpiar() {
        log.info("[retention] Iniciando limpieza programada");
        ...
        try {
            ...
            log.info("[retention] Completada: outbox_enviado={}, outbox_fallido={}, mensaje_procesado={}, duracion={}ms",
                enviados, fallidos, procesados, duracionMs);
        } catch (Exception e) {
            metrics.recordError();
            log.error("[retention] Error durante limpieza: {}", e.getMessage(), e);
        }
    }
    ...
    private int limpiarPorBatches(IntSupplier batchSupplier) {
        ...
        do {
            ...
            if (batches >= config.getMaxBatchesPerRun()) {
                log.warn("[retention] Techo de batches alcanzado ({}), pausando hasta proximo run", batches);
                break;
            }
