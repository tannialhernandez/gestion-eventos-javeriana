...
/**
 * Métricas Micrometer para el Outbox Relay de payment-service.
 *
 ...
 *  - outbox.events.failed.terminal   (Counter) : eventos marcados FALLIDO (agotados reintentos)
 *  - outbox.polling.batch.size       (Summary) : distribución del tamaño de lote por ciclo
 *
 * Consultar vía: GET /actuator/prometheus (o /actuator/metrics)
 */
@Component
public class OutboxMetrics {

    private static final String SERVICIO = "payment-service";
    ...
    public OutboxMetrics(MeterRegistry registry, OutboxEventRepository outboxRepository) {
        ...
        // Gauge: consulta BD en cada scrape de Prometheus → refleja estado real
